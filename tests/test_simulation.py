...
class TestMockReadingData:
    def test_returns_dataframe(
        self, negative_sim_data, positive_sim_data
    ):
        result = mock_reading_data(
            negative_sim_data, positive_sim_data
        )
    ...
    def test_has_expected_columns(
        ...
        assert MOCK_READING_EXPECTED_COLUMNS.issubset(
            result.columns
        )
    ...
    def test_reader_values_in_range(
        self, negative_sim_data, positive_sim_data
    ):
        n_readers = negative_sim_data["n_readers"]
        result = mock_reading_data(
        ...
        assert set(result["reader"].unique()) == set(
            range(n_readers)
        )

    def test_case_values_in_range(
        self, negative_sim_data, positive_sim_data
    ):
        n_cases = negative_sim_data["n_cases"]
        result = mock_reading_data(
        ...
        assert set(result["case"].unique()) == set(
            range(n_cases)
        )

    def test_correct_rating_count_negative(
        self, negative_sim_data, positive_sim_data
    ):
        ...
        for reader in range(negative_sim_data["n_readers"]):
            for treatment in [0, 1]:
                mask = (neg["reader"] == reader) & (
                    neg["treatment"] == treatment
                )
                n_correct = (
                    neg.loc[mask, "rating"] == 0
                ).sum()
                assert (
                    n_correct
                    == negative_sim_data["k"][
    ...
    def test_correct_rating_count_positive(
        self, negative_sim_data, positive_sim_data
    ):
        ...
        for reader in range(positive_sim_data["n_readers"]):
            for treatment in [0, 1]:
                mask = (pos["reader"] == reader) & (
                    pos["treatment"] == treatment
                )
                n_correct = (
                    pos.loc[mask, "rating"] == 1
                ).sum()
                assert (
                    n_correct
                    == positive_sim_data["k"][
...
def negative_case_sim_data():
    return simulate_case_data(
        n_readers=3, n_cases=5, true_params=CASE_TRUE_PARAMS
    )
...
def positive_case_sim_data():
    return simulate_case_data(
        n_readers=3, n_cases=4, true_params=CASE_TRUE_PARAMS
    )


class TestSimulateCaseData:
    def test_returns_dict(self):
        result = simulate_case_data(
            n_readers=3,
            n_cases=5,
            true_params=CASE_TRUE_PARAMS,
        )
        assert isinstance(result, dict)
    ...
    def test_reproducible_with_same_rng(self):
        r1 = simulate_case_data(
            n_readers=3,
            n_cases=5,
            true_params=CASE_TRUE_PARAMS,
            rng=np.random.default_rng(0),
        )
        r2 = simulate_case_data(
            n_readers=3,
            n_cases=5,
            true_params=CASE_TRUE_PARAMS,
            rng=np.random.default_rng(0),
...
class TestMockCaseReadingData:
    def test_returns_dataframe(
        self, negative_case_sim_data, positive_case_sim_data
    ):
        result = mock_case_reading_data(
            negative_case_sim_data, positive_case_sim_data
        )
        assert isinstance(result, pd.DataFrame)
    ...
    def test_has_expected_columns(
        ...
        assert CASE_READING_EXPECTED_COLUMNS.issubset(
            result.columns
        )
    ...
    def test_row_count(
        ...
        assert len(result) == 2 * n_readers * (
            n_cases_neg + n_cases_pos
        )

    def test_ratings_are_binary(
        self, negative_case_sim_data, positive_case_sim_data
    ):
        result = mock_case_reading_data(
            negative_case_sim_data, positive_case_sim_data
        )
        assert set(result["rating"].unique()).issubset(
            {0, 1}
        )
    ...
    def test_negative_case_indices(
        self, negative_case_sim_data, positive_case_sim_data
    ):
        ...
        assert set(neg_cases.unique()) == set(
            range(n_cases_neg)
        )

    def test_positive_case_indices(
        self, negative_case_sim_data, positive_case_sim_data
    ):
        ...
        assert set(pos_cases.unique()) == set(
            range(n_cases_neg, n_cases_neg + n_cases_pos)
        )
