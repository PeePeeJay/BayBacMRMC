...
PRIORS_OPTIONS = ["diffuse", "weakly informative", "informative", "frequentist"]
...
def _fake_summary(*args, **kwargs):
    ...
    return pd.DataFrame({"mean": [0.5, 0.2]}, index=["mu_a", "mu_b"])
...
class TestRunPsaCaseA:
    ...
    def setup(self, tmp_path):
        self.output_dir = str(tmp_path / "psa_results" / "estimates")
    ...
    def test_file_count(self):
        ...
        self._run()
        files = [f for f in os.listdir(self.output_dir) if f.endswith(".pkl")]
        expected = 1 * len(N_READERS_SIM) * len(GAMMA_SIM) * len(PRIORS_OPTIONS)
    ...
    def test_filename_format(self):
        ...
        for fname in os.listdir(self.output_dir):
            ...
            stem = fname.replace("estimate_", "").replace(".pkl", "")
            parts = stem.split(".")
    ...
    def test_frequentist_has_finite_intercept_and_slope(self):
        ...
        for prior_idx, prior in enumerate(PRIORS_OPTIONS):
            ...
            for size_idx in range(len(N_READERS_SIM)):
                for gamma_idx in range(len(GAMMA_SIM)):
                    ...
                    with open(os.path.join(self.output_dir, fname), "rb") as f:
                        est = pickle.load(f)
                    assert np.isnan(est["intercept_freq"]), fname
                    assert np.isnan(est["slope_freq"]), fname
            ...
            for size_idx in range(len(N_READERS_SIM)):
                for prior_idx in range(len(PRIORS_OPTIONS)):
                    ...
                    with open(os.path.join(self.output_dir, fname), "rb") as f:
                        est = pickle.load(f)
                    assert np.isclose(
                        est["true_params"]["gamma"], gamma_val
                    ), f"{fname}: expected gamma={gamma_val}, got {est['true_params']['gamma']}"

    def test_run_model_inference_called_for_bayesian_priors(self):
        """run_model_inference is called once per (size, gamma, Bayesian prior) combination."""
        n_bayesian = sum(1 for p in PRIORS_OPTIONS if p != "frequentist")
        expected_calls = 1 * len(N_READERS_SIM) * len(GAMMA_SIM) * n_bayesian
        with (
            patch(
                "mrmc_baybac.model_checking.run_model_inference",
                return_value=[MagicMock(), MagicMock()],
    ...
    def test_run_model_inference_called_with_correct_priors(self):
        """run_model_inference is called with each of the three Bayesian prior strings."""
        used_priors = set()
        original_patch = MagicMock(return_value=[MagicMock(), MagicMock()])
        ...
        expected_bayesian = {"diffuse", "weakly informative", "informative"}
...
class TestRunPsaCLI:
    ...
    def _run_cli(self, argv_extras=None):
        ...
        """
        with patch("mrmc_baybac.model_checking.run_psa") as mock_psa:
...
class TestRunPsaIntegration:
    ...
    def test_expected_file_count(self):
        ...
        self._run()
        files = [f for f in os.listdir(self.output_dir) if f.endswith(".pkl")]
        assert len(files) == 1 * 1 * len(self.GAMMA_SIM) * len(self.PRIORS_OPTIONS)
    ...
    def test_all_expected_files_present(self):
        ...
        for prior_idx in range(len(self.PRIORS_OPTIONS)):
            for gamma_idx in range(len(self.GAMMA_SIM)):
                ...
                assert os.path.isfile(os.path.join(self.output_dir, fname)), fname
    ...
    def test_bayesian_estimates_are_finite(self):
        ...
        for prior_idx in range(len(self.PRIORS_OPTIONS)):
            ...
            for gamma_idx in range(len(self.GAMMA_SIM)):
                ...
                with open(os.path.join(self.output_dir, fname), "rb") as f:
                    est = pickle.load(f)
                for key in ("mu_a_neg", "mu_b_neg", "mu_a_pos", "mu_b_pos"):
                    assert np.isfinite(est[key]), f"{fname}: {key} is not finite"
    ...
    def test_frequentist_estimates_are_finite(self):
        ...
        freq_idx = self.PRIORS_OPTIONS.index("frequentist")
        for gamma_idx in range(len(self.GAMMA_SIM)):
            fname = f"estimate_0.{freq_idx}.0.{gamma_idx}.pkl"
            with open(os.path.join(self.output_dir, fname), "rb") as f:
                est = pickle.load(f)
            assert np.isfinite(est["intercept_freq"]), fname
            assert np.isfinite(est["slope_freq"]), fname
    ...
    def test_mu_baseline_and_effect_size_stored(self):
        ...
        self._run(["--mu-baseline", "0.75", "--effect-size", "0.15"])
